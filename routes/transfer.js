...
router.post("/transfer", function (req, res) {
  ...
  userModel.findOne(
    { "bio.login.username": req.body.user },
    {
      "bio.email": 1,
      "account.currency": 1,
    ...
    function (err, doc) {
      ...
      if (restrictedStatus) {
        ...
        if (req.body.amount > acctBalance) {
          ...
          return res.send({
            error:
              "You have insufficient funds to effect this transfer, kindly fund your account and try again. Your account balance is " +
              currency +
                ...
                if (err) {
                  ...
                  sendEmailNotification(mailOptions).then(
                    (data) => {
                      return res.send({
                        success:
                          "Your transfer has been initiated successfully, if anything goes wrong, we'll notify you by email, Thank you for banking with us. Your account balance is " +
                          currency +
                    ...
                    (error) => {
                      ...
                      return res.send({
                        error:
                          "This process was completed successfully, but there was an error sending your confirmation email.",
                      });
                    }
                  );
                }
